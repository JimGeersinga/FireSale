...
public interface AuctionRepository extends JpaRepository<Auction, Long> {
    ...
    @Query("SELECT a FROM Auction a WHERE a.status = 'READY' AND a.startDate <= current_timestamp AND a.endDate > current_timestamp")
    ...
    @Query("SELECT a FROM Auction a WHERE a.status = 'READY' AND a.startDate <= current_timestamp AND a.endDate > current_timestamp AND a.isFeatured = true")
    List<Auction> findActiveAuctionsByIsFeaturedTrue();

    @Query("SELECT distinct a FROM Auction a join a.categories c WHERE a.status = 'READY' AND a.startDate <= current_timestamp AND a.endDate > current_timestamp AND c.id in :categories")
    List<Auction> findAuctionsByCategories(long[] categories);

    @Query("SELECT distinct a FROM Auction a join a.tags c WHERE a.status = 'READY' AND a.startDate <= current_timestamp AND a.endDate > current_timestamp AND c.id in :tags")
    List<Auction> findAuctionsByTags(long[] tags);

    @Query("SELECT distinct a FROM Auction a WHERE a.status = 'READY' AND a.startDate <= current_timestamp AND a.endDate > current_timestamp AND a.name LIKE CONCAT('%',:name,'%')")
    ...
    @Query("SELECT distinct a FROM Auction a join a.tags t " +
            "WHERE a.status = 'READY' AND " +
            "a.startDate <= current_timestamp AND a.endDate > current_timestamp AND " +
            "a.name LIKE CONCAT('%',:name,'%') AND t.id in :tags")
    ...
    @Query("SELECT distinct a FROM Auction a join a.tags t join a.categories c " +
            "WHERE a.status = 'READY' AND " +
            "a.startDate <= current_timestamp AND a.endDate > current_timestamp AND " +
            "a.name LIKE CONCAT('%',:name,'%') AND t.id in :tags AND c.id in :categories")
    List<Auction> findAuctionsByTagsLikeAndCategoriesANDNameLike(long[] tags, long[] categories, String name);

    @Query("SELECT distinct a FROM Auction a join a.tags t join a.categories c " +
            "WHERE a.status = 'READY' AND " +
            "a.startDate <= current_timestamp AND a.endDate > current_timestamp AND " +
            "t.id in :tags AND c.id in :categories")
